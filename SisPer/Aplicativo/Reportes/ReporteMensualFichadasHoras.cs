...
{
    public struct Datos_informe_fichadas_mensuales
    {
    ...
    public class ReporteMensualFichadasHoras : Informe_Personal<Datos_informe_desde_hasta<Datos_informe_fichadas_mensuales>>
    {
        public ReporteMensualFichadasHoras(Datos_informe_desde_hasta<Datos_informe_fichadas_mensuales> datos, Agente ag_informe) : base(datos, ag_informe, "PLANILLA DE ASISTENCIA")
        {
        }

        protected override stream_informe Generar_cuerpo_informe(Datos_informe_desde_hasta<Datos_informe_fichadas_mensuales> datos, stream_informe informe)
        {
            Document document = informe.document;

            //Genero el PDF en memoria para ir agregando las partes
            int corrida = 0;
            DateTime primer_dia_mes = datos.desde;
            DateTime ultimo_dia_mes = datos.hasta;

            foreach (Agente agente in datos.datos.Agentes)
            {
                ...
                Table tableEncabezado = new Table(UnitValue.CreatePercentArray(new float[] { 80, 20 })).UseAllAvailableWidth().SetFontSize(8).SetMarginBottom(5);

                Cell celldaEncabezado = new Cell(1, 1).Add(new Paragraph(agente.Area.Nombre.ToString().ToUpper()))
                                    .SetFontSize(10)
                                    .SetBold()
                                    .SetTextAlignment(TextAlignment.CENTER);
                tableEncabezado.AddCell(celldaEncabezado);

                celldaEncabezado = new Cell(1, 1).Add(new Paragraph(primer_dia_mes.ToString("MMMM").ToUpper() + " " + primer_dia_mes.ToString("yyyy")))
                ...
                Table table = new Table(UnitValue.CreatePercentArray(new float[] { 20, 20, 20, 95, 20, 35 })).UseAllAvailableWidth().SetFontSize(9);
                ...
                int cantidad_dias_franco_compensatorio = 0;
                        ...
                        if (rd != null && rd.Cerrado == true)
                        {
                            if (rd.Marcaciones != null && rd.Marcaciones.Count == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (estado != "Razones particulares")
                                    {
                                    ...
                                    switch (estado)
                                    {
                                        case "Franco compensatorio":
                                            cantidad_dias_franco_compensatorio++;
                                            break;
                                        case "Licencia Anual":
                ...
                total_periodo_horas = HorasString.RestarHoras(total_periodo_horas_mas, total_periodo_horas_menos);
                ...
                cell = new Cell(1, 9).SetBorder(iText.Layout.Borders.Border.NO_BORDER).SetFontSize(6); table.AddCell(cell);
                cell = new Cell(1, 3).Add(new Paragraph(String.Format("Días licencia anual: {0}", cantidad_dias_licencia_anual.ToString()))).SetBorder(iText.Layout.Borders.Border.NO_BORDER); table.AddCell(cell);
                cell = new Cell(1, 3).Add(new Paragraph(String.Format("Días licencia anual (SALDO): {0}", cantidad_dias_licencia_anual_saldo.ToString()))).SetBorder(iText.Layout.Borders.Border.NO_BORDER); table.AddCell(cell);
                cell = new Cell(1, 3).Add(new Paragraph(String.Format("Días franco compensatorio: {0}", cantidad_dias_franco_compensatorio.ToString()))).SetBorder(iText.Layout.Borders.Border.NO_BORDER); table.AddCell(cell);
                cell = new Cell(1, 3).Add(new Paragraph(String.Format("Días licencia especial invierno: {0}", cantidad_dias_licencia_invierno.ToString()))).SetBorder(iText.Layout.Borders.Border.NO_BORDER); table.AddCell(cell);
                cell = new Cell(1, 3).Add(new Paragraph(String.Format("Días licencia enfermedad: {0}", cantidad_dias_licencia_enfermedad.ToString()))).SetBorder(iText.Layout.Borders.Border.NO_BORDER); table.AddCell(cell);
                cell = new Cell(1, 3).SetBorder(iText.Layout.Borders.Border.NO_BORDER); table.AddCell(cell);
